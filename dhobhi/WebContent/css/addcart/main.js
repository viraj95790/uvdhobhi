...
jQuery(document).ready(function($){
	...
	function addProduct() {
		...
		cartList.prepend(laundrycartproductdata);
	}
	...
	function addlaundrycartAjax(){
		
		var serviceid = document.getElementById('serviceid').vlaue;
	 ...
	 var url ="cartService?serviceid="+serviceid+"&id="+item+"&subitem="+subitem+"&pickup_date="+pickup_date+"&qty="+qty+"&deliver_date="+deliver_date+"&ptime="+ptime+"&dtime="+dtime+" ";
		 ...
		 if (window.XMLHttpRequest) {
		    ...
		    req.onreadystatechange = addlaundrycartAjaxRequest;
		    req.open("GET", url, true); 
		              
		    req.send(null);
	}

	function addlaundrycartAjaxRequest(){
		
		if (req.readyState == 4) {
			if (req.status == 200) {
				
				
				laundrycartproductdata = req.responseText;
				
				cartList.prepend(laundrycartproductdata);
				
				document.getElementById('chkoutamtid').innerHTML = document.getElementById('hdnchkoutamtid').value;
...
function deletecartitem(id){
	...
	var url = "deletecartService?selectedid="+id+" ";
	 ...
	 if (window.XMLHttpRequest) {
	    ...
	    req.onreadystatechange = deletecartRequest;
	    req.open("GET", url, true); 
